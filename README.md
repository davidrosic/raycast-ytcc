...
Open **YouTube Download** under Raycast's Development section. If your clipboard already holds a YouTube watch, short, live, embed, or `youtu.be` link, the command fills in the link and searches right away. Pasting a YouTube link into the field with ⌘V also searches right away. Typed links and links to other sites wait for ⌘↵. The results open at once with the video's thumbnail, title, channel, duration, and upload date in a side panel, so you can confirm it is the right video while the caption list loads. Use the format selector above the language list, then press Return on a language to save it. RAW is the default: it saves caption text in its original cue order, including repeated rolling cues, as a file ending `- RAW.txt`. Clean TXT removes repeated rolling text. SRT and VTT keep timestamps. To download the same video's media, choose MP3, M4A, or MP4 in the **Audio & Video** section. Links from other sites supported by `yt-dlp` also work when you search them manually. Files are saved to `~/Downloads` by default; you can choose another folder in extension preferences. Existing files are given a numeric suffix.
